...
class ST_EventMatcher:
    def __init__(self,  events_l,events_r,frameTime, shape, time_threshold=1, spatial_threshold=1,disparity=50):
    ...
    def __next__(self):
       ...
       # same_frame=find_matching_coordinates(frame1,frame2)
        ...
        frame_l_match = np.array(frame_l_match)
        #points = np.array(frame_l_match)[:, :2]
        dbscan = DBSCAN(eps=3, min_samples=3)  # eps和min_samples根据数据集进行调整
        clusters = dbscan.fit_predict(np.array(frame_l_match)[:, :2])
        filtered_points = frame_l_match[clusters != -1]
        for event_m in filtered_points:
        ...
        if first_event_time < 0:
            raise StopIteration
        return frame1,frame2,filtered_points
    ...
    def match_event(self, left_event):
        ...
        match_event=[0,0,0,0]
        for i in range(-1,1):
            for j in range(x-self.disaprity,x+self.disaprity):
                if(j>=640 or j<0): continue
                time_difference = abs(self.spike_map[y+i,j][0][0] - t)
                disparity_distance=abs(x-j)
                if p == self.spike_map[y+i,j][0][1] and time_difference<=self.time_threshold:
                    cost=abs(i)+float(time_difference/1000)
                    if cost<all_cost:
                        all_cost=cost
                        match_event[0]=j
                        match_event[1]=y+i
                        match_event[2]=disparity_distance
                        match_event[3]=p
                        #return match_event
