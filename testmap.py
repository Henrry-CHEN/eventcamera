...
filename_last ="D:/PycharmProjects/pythonProject/event7.aedat4"
...
def main():
    with AedatFile(filename_last) as f:
        ...
        event_matched=ST_ex.ST_EventMatcher(f['undistortedEvents'],f['undistortedEvents_1'],5,(480,640),2)
        #event_matched = ST_before.ST_EventMatcher(f['undistortedEvents'], f['undistortedEvents_1'], 2, (480, 640), 2)
        ...
        i=0
        for frame1, frame2,points in event_matched:
            starttime = time.perf_counter()
            gray_left_im = frametoimage(frame1)
            gray_right_im = frametoimage(frame2)
            alpha = 0.05  # Alpha值的选择取决于点集和期望的形状精细度
            alpha_shape = alphashape.alphashape(points[:, :2], alpha)
            depths = points[:, 2]
            depths_value=np.mean(depths)
            depths=remove_outliers(depths)
            depth_value=5000/np.mean(depths)
            endtime = time.perf_counter()
            timediff = endtime - starttime
            print(timediff)
            cmap = plt.get_cmap('viridis')
            color = cmap(depth_value)
            if isinstance(alpha_shape, Polygon):
                # Alpha Shape是单个多边形
                fig, ax = plt.subplots(figsize=(640/96, 480/96), dpi=96)
                ...
                for polygon in alpha_shape.geoms:
                    ...
                    ax.fill(x, y, color=color)
            ax.set_xlim([0, 640])
            ax.set_ylim([480, 0])  # Y轴坐标翻转

            # 隐藏坐标轴的刻度
            ax.set_xticks([])
            ax.set_yticks([])
            filename_map = f'./depth/sin_plot_{i + 1}.png'
